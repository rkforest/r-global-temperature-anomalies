...
save_csv_dir <- file.path("data", "1-raw-data", "csv")
save_rds_dir <- file.path("data", "1-raw-data", "rds")
...
fn_get_df <- function(url, skip_header_recs) {
...
for (i in 1:length(urls)) {
  df <- fn_get_df(urls[i], skip_header_recs[i])
  df_list[[i]] <- df
}
